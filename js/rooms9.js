    ...
    групи.forEach(([загл, неща]) => {
      ...
      неща.forEach((н, i) => {
        ...
        r.addEventListener('click', () => {
          ...
          const вкл = !мои[к];
          мои[к] = вкл;
          // 🔴 25.08 (dev/pylna_pamet.js, живо натискане при пълна памет): отметката
          //    светваше „✔" и при паднал запис — спешната чанта изглеждаше събрана,
          //    а при следващото отваряне беше празна. Общ помощник: този един ред
          //    пази ВСИЧКИ чеклисти в файла. `save` вече вика BL_ZAPIS_PADNA (ред 14).
          if (!save(ключ, мои)) return;
  ...
  function loadCard() {
    ...
    function действия(дял, брой_отметки) {
      ...
      зап.addEventListener('click', () => {
        ...
        const д = today();
        // 🔴 25.08 (dev/lazhliv_uspeh.js): отговорът на записа се хвърляше и
        //    „✔ Записано: N%" излизаше и при пълна памет. И екранът НЕ се
        //    самопоправя: `рисувайИст()` чете масива `ист` от паметта, не склада —
        //    тоест числото стоеше на екрана, а в телефона го нямаше.
        //    Сега: смятаме настрани, записваме, и чак при потвърден запис
        //    пипаме `ист` и екрана. `save` вече казва истината (rooms9.js:14).
        const нова = ист.slice();
        if (нова.length && нова[нова.length - 1] && нова[нова.length - 1].d === д) нова[нова.length - 1] = { d: д, v: сегашен };
        else нова.push({ d: д, v: сегашен });
        const орязана = нова.slice(-12);
        if (!save('bl_wm_load_hist', орязана)) return;
        ист.length = 0; орязана.forEach(x => ист.push(x));   // същият масив — рисувайИст() го държи в затваряне
        рисувайИст(); пулс(редИст);
  ...
  function partnerCard() {
    ...
    if (!ОТГОВОР) {
      ...
      не.addEventListener('click', () => {
        // 🔴 25.08 (dev/lazhliv_uspeh.js): изборът ѝ се хвърляше и картата се
        //    изчистваше дори когато „не"-то не е стигнало до паметта. Тогава
        //    „няма да я виждаш повече" е неистина — при следващото отваряне
        //    въпросът се връща. `save` вече казва истината (rooms9.js:14).
        if (!save('bl_partner', 'не')) return;
        п.textContent = '';
