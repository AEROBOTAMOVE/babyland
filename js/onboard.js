...
(function () {
  ...
  function close() {
    ...
    try { if (window.BL_NAV2 && BL_NAV2.обновиБутона) BL_NAV2.обновиБутона(); } catch (e) {}   // 15.09 dete2g-07: долният бутон според възрастта
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    $('onbSave').addEventListener('click', () => {
      ...
      // 15.09 (E2E „трудни случаи“, T1): толерансът „+ 1 ден“ пропускаше УТРЕШНАТА дата —
      //   „2026-09-16“ се чете като полунощ UTC и е по-малко от „сега + 24 ч“. Бебето оставаше
      //   без възраст (BL_AGE null). Сравняваме НИЗ с НИЗ, както за рождения ден на мама.
      //   T7: дата отпреди повече от 5 години почти винаги е сгрешена година — а приложението
      //   е за първите 3 години и би давало съвети за бебе на петгодишно дете.
      if (!waiting && $('onbDate').value) {
        if ($('onbDate').value > днесНиз()) {
          return спри($('onbDate'), 'Тази дата е в бъдещето 💛 Ако още чакаш — избери „Още чакам“.');
        }
        const преди5 = String(+днесНиз().slice(0, 4) - 5) + днесНиз().slice(4);
        if ($('onbDate').value < преди5) {
          return спри($('onbDate'), 'Тази дата е преди повече от 5 години 💛 Провери годината — тук сме за първите три години.');
        }
      }
