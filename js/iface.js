...
(function () {
  ...
  function облечиТабове(стая) {
    ...
    //    мама отвори рафта.
    if (табове._blПазач) табове._blПазач.disconnect();
  ...
  function обновиЛента(стая) {
    ...
    if (!toc) return;
    const всички = document.querySelectorAll('#roRoom .jr-card[data-blkey]').length;
    if (!всички) return;
    const бр = Object.keys((load(ВИДЕНИ, {})[стая]) || {}).length;
    const н = Math.min(бр, всички);
