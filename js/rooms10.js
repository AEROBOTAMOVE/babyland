    ...
    КУПУВАНЕ.forEach(([загл, неща]) => {
      ...
      неща.forEach((н, i) => {
        ...
        r.addEventListener('click', () => {
          ...
          сега[к] = !сега[к];
          // 🔴 25.08 (dev/pylna_pamet.js, пълна памет): „✔" се пускаше и когато
          //    записът е паднал — списъкът за купуване изглеждаше отметнат, а на
          //    другия ден беше празен. Отметката тръгва само след потвърден запис.
          if (!save('bl_buy', сега)) return;
    ...
    Д.forEach(([загл, неща]) => {
      ...
      неща.forEach((н, i) => {
        ...
        r.addEventListener('click', () => {
          ...
          сега[к] = !сега[к];
          // 🔴 25.08 (dev/pylna_pamet.js, пълна памет): същото и тук — документите
          //    се отмятаха на екрана, без да влизат в паметта.
          if (!save('bl_docs', сега)) return;
          r.classList.toggle('done', !!сега[к]);
          r.setAttribute('aria-pressed', сега[к] ? 'true' : 'false');
          r.querySelector('.jr-check').textContent = сега[к] ? '✔' : '';
          пребрий(); fx().buzz(5);
