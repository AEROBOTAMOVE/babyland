...
(function () {
  ...
  function diapersTotalCard() {
    ...
    const дниЗапис = Object.keys(d).filter(k => { const x = d[k] || {}; return (+x.wet || 0) + (+x.dirty || 0) > 0; });
    ...
    c.appendChild(el('p', 'jr-privacy',
      `Отбелязани за <strong>${дни}</strong> ${дни === 1 ? 'ден' : 'дни'} · средно <strong>${средно}</strong> на ден.<br>
       По около 3 минути всяка — това са <strong>${часове} часа</strong> от живота ти. Никой няма да ти благодари за тях. Аз ти благодаря.`));
    const b = el('button', 'jr-chip', '📤 Сподели числото'); b.type = 'button';
    b.addEventListener('click', () => {
      const т = `${общо} пелени. ${часове} часа. И нито едно „благодаря“. 😄 (Бейби Ленд)`;
  ...
  function askPedCard() {
    ...
    const мои = load('bl_qped', {});
    набор[2].forEach((q, i) => {
      const к = набор[0] + '|' + i;
      const r = el('button', 'qd-row' + (мои[к] ? ' done' : '')); r.type = 'button';
      r.innerHTML = `<span class="jr-check">${мои[к] ? '✔' : ''}</span><span>${esc(q)}</span>`;
      r.addEventListener('click', () => {
        мои[к] = !мои[к]; save('bl_qped', мои);
        r.classList.toggle('done'); r.querySelector('.jr-check').textContent = мои[к] ? '✔' : '';
      });
      c.appendChild(r);
    });
  ...
  function booksCard() {
    ...
    КНИГИ.forEach(([x, y, e, име, оп]) => {
      ...
      r.innerHTML = `<span class="bk-e">${e}</span><span class="bk-t"><strong>${esc(име)}</strong><br><small>${esc(оп)}</small></span>`;
      c.appendChild(r);
    });
    c.appendChild(el('p', 'jr-privacy',
  ...
  function docsCard() {
    ...
    const Д = [
      ...
      ['Първите месеци', ['Детски надбавки — ако отговаряте на условията', 'Лична карта/паспорт — ако ще пътувате', 'Записване в детска градина (в някои градове се чака година+)']]
    ];
