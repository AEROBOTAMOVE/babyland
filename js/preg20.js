...
(function () {
  ...
  function третиТриместърКарта(w) {
    ...
    c.innerHTML = `<h4 class="jr-title">Третият триместър идва 📋 <span class="jr-sub">пет неща, които се мислят отсега</span></h4>`;
    const т = load('bl_tri3', {});
    ['Курс за раждане / дишане — записани ли сме?', 'Столчето за колата — избрано/монтирано?',
     'Кой ще гледа голямото дете / кучето в деня Х?', 'Пътят до болницата — кой кара, колко време е?',
  ...
  function пътекаКарта(root, w) {
    ...
    c.innerHTML = `<h4 class="jr-title">Пътеката на чакането 🎡 <span class="jr-sub">40 седмици — ти си на ${w > 0 ? w + '-та' : 'старта'}</span></h4>`;
    ...
    svg.innerHTML = `<svg viewBox="0 0 300 150" class="pg20-svg" role="img" aria-label="Пътят на бременността — седмица ${w} от 40">
  ...
  function надгради(root) {
    ...
    if (музика && гласово && !музика.querySelector('.pg20-link')) {
      const към = (от, до, текст) => {
        ...
        b.addEventListener('click', () => { до.classList.remove('folded'); до.scrollIntoView({ behavior: 'smooth', block: 'start' }); });
        от.appendChild(b);
      };
