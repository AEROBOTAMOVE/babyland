...
(function () {
  ...
  function седмица() {
    ...
    //    Старият израз мереше UTC полунощ срещу тукашно време — в София 2-3
    //    часа разлика — и всеки седми ден между 00:00 и 02:00 даваше ЕДНА
    //    СЕДМИЦА ПО-МАЛКО (измерено: 1.6% от 5760 мига), а в самия ден на ПМ
    //    даваше −1. BL_PREG брои КАЛЕНДАРНИ дни между две МЕСТНИ полунощи.
    //    Резервният изход остава НАРОЧНО: не се ли зареди data.js, поведението
    //    е точно старото, а не празно. ПЪТ НАЗАД: махни лявата страна на `?`.
    return (window.BL_PREG ? BL_PREG.седмица(d) : Math.floor((Date.now() - d) / 604800000));
  }
  function дниДоТермин() {
    const l = lmp(); if (!l) return null;
    // и тук едно число за целия проект: „остават N дни" се смяташе с Math.ceil
    // на едни места и Math.round на други — измерено разминаване в 50% от миговете.
    return (window.BL_PREG ? BL_PREG.доТермина(l) : Math.ceil((new Date(l).getTime() + 280 * 86400000 - Date.now()) / 86400000));
  }
  ...
  function раждаКарта() {
    ...
    б.addEventListener('click', () => {
      ...
      текущо.birth = d;
      // 🔴🔴 25.08 (dev/lazhliv_uspeh.js) — НАЙ-СКЪПОТО място в този файл:
      //    отговорът на записа се хвърляше. При пълна памет рожденият ден НЕ се
      //    записваше, а редът отдолу ТРИЕШЕ `bl_lmp` — тоест бременността си
      //    отиваше, бебето го нямаше, конфетите падаха и стаята се сменяше с
      //    празна. Загуба в двете посоки от едно натискане.
      //    Нищо не се трие и нищо не се празнува, докато рожденият ден не е вътре.
      //    `save` вече вика BL_ZAPIS_PADNA (preg20.js:23).
      if (!save('bl_baby', текущо)) {
        знак(вест, 'Не можах да го запазя — датата ти стои в полето. Освободи малко място и натисни пак.');
        return;
      }
      try { localStorage.removeItem('bl_lmp'); } catch (e) {}   // бременността приключи (както onboard.js)
  ...
  function поканаКарта(root) {
    ...
    б.addEventListener('click', () => {
      ...
      // 🔴 25.08 (dev/lazhliv_uspeh.js): „✔ Записах. Отварям стаята ти…" излизаше
      //    и при паднал запис — стаята се отваряше празна, все едно нищо не е било.
      if (!save('bl_lmp', п.value)) {
        знак(вест, 'Не можах да го запазя — датата ти стои в полето. Освободи малко място и натисни пак.');
        return;
      }
  ...
  function прегледКарта(w) {
    ...
    зап.addEventListener('click', () => {
      ...
      // 🔵 25.08: тук екранът СЕ самопоправя — `рисувайДата()` чете bl_events
      //    НАНОВО от паметта, тоест при паднал запис надписът пак ще каже
      //    „Запиши датата". Остава само вибрацията да не казва „стана", когато
      //    не е станало: числото и думата до него не бива да си противоречат.
      if (!save('bl_events', ev)) { рисувайДата(); return; }
      fx().buzz(10); рисувайДата();
    });
    махни.addEventListener('click', () => {
      // 🔴 25.08 (dev/lazhliv_uspeh.js): `рисувайДата()` чете НАНОВО от паметта и
      //    сам би казал истината — но редът отдолу ПРЕЗАПИСВА надписа му с
      //    „↩ Махнах датата". При паднал запис прегледът ѝ си стоеше, а на екрана
      //    пишеше, че го няма. Сега махането се обявява само ако е станало.
      if (!save('bl_events', load('bl_events', []).filter(x => !x.preg))) { рисувайДата(); return; }
    ...
    своя.addEventListener('input', () => {
      // 🔴 25.08 (dev/lazhliv_uspeh.js): отговорът се хвърляше и „✔ Записано —
      //    ще те чака тук." излизаше при всяка буква, включително при пълна
      //    памет. Полето НЕ се чисти тук (текстът ѝ остава), но обещанието беше
      //    невярно. Сега надписът казва каквото е станало.
      //    `save` вече вика BL_ZAPIS_PADNA (ред 23) — а той се обажда веднъж на
      //    5 секунди, тоест не крещи на всеки натиснат клавиш.
  ...
  function сигналиКарта() {
    ...
    c.innerHTML = `<h4 class="jr-title">Сигнали, които не чакат 🚨 <span class="jr-sub">спокойно, но веднага — звънни на лекаря си</span></h4>
      <ul class="pg20-siglist">
        ...
        <li>🤫 Бебето утихна — движенията рязко намаляха (след 28-ма)</li>
  ...
  function споменКарта(w) {
    ...
    б.addEventListener('click', () => {
      ...
      // 🔴 25.08 (dev/lazhliv_uspeh.js): и двата записа хвърляха отговора си, а
      //    отдолу пишеше „💜 Прибрах го. После ще ти го върна." При пълна памет
      //    споменът ѝ не отиваше никъде — а тя си мислеше, че е прибран.
      //    Споменът е ГЛАВНИЯТ запис: не мине ли, не се обявява нищо.
      //    `save` вече вика BL_ZAPIS_PADNA (ред 23).
      запазени[ключ] = т;
      if (!save('bl_preg_memories', запазени)) {
        знак(вест, 'Не можах да го запазя — изречението ти стои в полето. Освободи малко място и натисни пак.');
        return;
      }
      ...
      // Реката е ВТОРИЧНА — споменът вече е вътре, текстът ѝ не се губи. Но
      // „После ще ти го върна." е обещание точно на Реката: даваме го само ако
      // тя наистина го е поела.
      const окРека = save('bl_river_manual', река);
      ...
      знак(вест, старо
        ? '✔ Промених го.'
        : (окРека ? '💜 Прибрах го. После ще ти го върна.' : '💜 Прибрах го.'));
      fx().buzz(10);
    });
    c.appendChild(п); c.appendChild(б); c.appendChild(вест);
    return c;
  ...
  function подредиПоСедмица(root, w) {
    ...
    if (w > 0 && w < 28) сгъни('План за раждане', '· към 28-ма');
    ...
    if (w > 0 && w < 28) сгъни('Чанта за родилния дом', '· към 28-ма');
  ...
  function дефектнаДатаКарта() {
    const c = el('section', 'jr-card pg20-hero');
    // 🔴 26.08 (ИЗМЕРЕНО, dev/kriv_zapis.js): при крив bl_lmp тук се
    //    показваше СУРОВИЯТ запис на майката — буквално `{"done":null}` в
    //    изречение, което трябва да ѝ помогне. Вътрешен боклук на екрана не
    //    обяснява нищо, а я плаши. Показваме дата само ако прилича на дата.
    //    ПЪТ НАЗАД: върни `${esc(l)}` и махни проверката.
    const l = lmp();
    const датаНаВид = (typeof l === 'string' && /^d{4}-d{2}-d{2}/.test(l)) ? l : null;
    c.innerHTML = `<h4 class="jr-title">Тази седмица 🌅 <span class="jr-sub">не мога да я сметна</span></h4>
      <p class="pg20-big">${датаНаВид ? 'По датата, която пазя (<strong>' + esc(датаНаВид) + '</strong>), излизаш' : 'По записаното засега излизаш'} извън първите ${МАКС_С} седмици. Най-често е сбъркана година при въвеждането.</p>
