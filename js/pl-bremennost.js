...
(function () {
  ...
  function поканаHTML() {
    ...
    return '<div class="pl-br-card pl-br-inv pl-felt">' +
      ...
      // 🪤 23.09 (МЕРЕНО, проверката): в поканата нямаше нито 112, нито дисклеймъра — а правило 4 иска
      //   112 ВИНАГИ видимо И „Записките не заменят преглед.“ навсякъде, където каним да се пише.
      //   Тук каним точно това: плочките отдолу („Моят календар“, „Въпроси към лекаря — Запиши за
      //   следващия преглед“) са видими и в поканата. Затова подножието е ДУМА ПО ДУМА същото като в
      //   картата „Предстоящи записи“ — един и същ ред на двата екрана, без втори прочит.
      //   Мерено 23.09 на 390 px: надписът е 190.6 px, бутонът 80.1 px в ред от 328 px → един ред.
      '<p class="pl-br-foot"><span>Записките не заменят преглед.</span><a class="pl-br-112" href="tel:112" aria-label="Спешна помощ 112">📞 112</a></p>' +
    '</div>';
  }

  // ═══ 4. ТРИТЕ ПЛОЧКИ — всяка е малка сцена от 2–3 плюшени предмета (реф. 4) ═══
  //   м = мястото в рамката: g голямото (в средата), l долу вляво, r долу вдясно (по-малко).
  //   Трите места са ЕДИНСТВЕНИТЕ, които css/pl-bremennost.css познава — четвъртото („m“) беше
  //   мъртво правило и е махнато 23.09.
  function плочкиHTML() {
