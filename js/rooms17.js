...
(function () {
  ...
  function rangesCard() {
    ...
    function рисувай() {
      ...
      r.innerHTML = `<span class="rg-e">${д.e}</span>
        ...
        <button class="rg-mark" type="button" title="отбележи кога стана">${моя != null ? '✔' : '+'}</button>`;
  ...
  const ДВУЕЗИЧНО = [
    ...
    ['👤', 'Един човек — един език', 'Най-работещият модел: мама говори единия, тати другия, последователно. Детето свързва езика с човек, не със ситуация.'],
    ['🏠', 'Или: у дома единия, навън другия', 'Другият работещ модел, ако и двамата родители говорите един и същ майчин език, а средата е друга.'],
    ['💪', 'Слабият език иска повече', 'Езикът, който детето чува по-малко, има нужда от повече часове — книжки, видео, роднини, игри. Иначе тихо избледнява.'],
  ...
  function typicalDayCard() {
    ...
    if (храна.length >= 5) {
      ...
      if (дни) редове.push(['🤱', 'хранения', Math.round(храна.length / дни) + ' пъти', 'средно на ден, от ' + дни + ' дни']);
    }
