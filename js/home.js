...
(function () {
  ...
  // 🤍 11.08 (ЖЕСТОКОТО): близнакът на поправката в персоПриветствие() по-долу.
  //    Поздравът минава през BL_EXPECT и се гаси при загуба — а муркането не.
  //    Жена, спряла броенето, стоеше на началния екран и на всеки ~7 секунди
  //    едно балонче я питаше „Ритна ли днес? 🌸“. Питаме за ритник само когато
  //    вратарят казва, че има какво да се пита; иначе Мила просто е тук.
  const МУРКИ = {
  ...
  function монтирайМуркане() {
    ...
    setInterval(() => {
      ...
      let реплика = МУРКИ[име] || 'Тук съм! 🎈';
      if (име === 'Мила' && !(window.BL_EXPECT ? BL_EXPECT.lmp() : '')) реплика = 'Тук съм. 🌸';
      const мех = el('span', 'мур', реплика);
  ...
  function mountMascot() {
    ...
    // 🐢 11.08 (бавен телефон): този тик буди процесора 75 пъти в минута ЗАВИНАГИ
    //   — и докато телефонът е в джоба със заключен екран. Другите тикове в този
    //   файл вече имат тази защита („работа за нищо, точно от рода, който изяжда
    //   батерията на слаб Android“ — mountFlip по-долу); този беше пропуснат.
    //   Щом мама се върне, visibilitychange рисува веднага — нищо не закъснява.
    //   ПЪТ НАЗАД: махни двата реда с `document.hidden`.
    const синхронизирай = () => {
      const ov = document.getElementById('roomOverlay');
      m.classList.toggle('hide', !!(ov && !ov.hidden));
    };
    const watch = setInterval(() => { if (document.hidden) return; синхронизирай(); }, 800);
    document.addEventListener('visibilitychange', () => { if (!document.hidden) синхронизирай(); });
  }
  ...
  function mountFaq() {
    ...
    // 🟠 11.08 (обиколка „приемането за дадено“): отговорът на „За кого е?“
    //    обещаваше „и за тати има място (писмата между вас 💌)“ — а картата
    //    „Писма между нас“ се СКРИВА при „🤍 Сама съм“ (rooms3.js:267). Тоест
    //    началният екран рекламираше на жена, която чака сама, точно това, което
    //    приложението после ѝ отказва. Функцията си остава за която я има;
    //    сменя се само какво ѝ се обещава. Празно = още не е казала → старият текст.
    //    ПЪТ НАЗАД: махаш `сама` и `ЗА_КОГО_САМА` — FAQ пак върви както е записан.
    const сама = load('bl_partner', '') === 'не';
    const ЗА_КОГО_САМА = 'От „чакаме бебе!“ до първите години — за теб преди всичко. И за хората, които ви обичат: готова картичка за баба с всичко важно 😄, писма, които се пазят за после 💌.';
    FAQ.forEach(([q, a]) => {
      if (сама && q === 'За кого е?') a = ЗА_КОГО_САМА;
