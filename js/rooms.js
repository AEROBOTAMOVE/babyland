...
(function () {
  ...
  //    масив-срещу-обект. Ключ с ЧИСЛО по подразбиране — точно bl_metime_start = 0
  //    — приемаше {} спокойно; после „сега минус {}" даваше NaN и на екрана на
  //    мама светеше часовник „NaN:NaN" в „Моето време". простФормат пази и
  //    трите прости вида. Числов низ („15") се ПРЕВРЪЩА, не се хвърля — стари
  //    версии са пазили числа като низове и изхвърлянето би загубило истински
  //    данни. Връща undefined, когато няма мнение — не null, защото null е
  //    законна стойност по подразбиране на много места тук.
  ...
  const load = (k, d) => {
    try {
      ...
      const прим = простФормат(v, d);
      if (прим !== undefined) return прим;
  ...
  function renderJournal(root) {
    ...
    tearBtn.type = 'button';
    // 🟢 26.08 — dev/pylna_pamet.js обявява този бутон за „ЧИСТИ ПОЛЕТО при
    //    ПЪЛНА ПАМЕТ". ИЗМЕРЕНО е, че НЕ Е дефект, и затова НЕ се пипа:
    //      · натиснат с текст в полето → мърдат ТОЧНО ДВА ключа, и двата чужди
    //        счетоводни: bl_carduse (брояч на ползване) и bl_bub_offer
    //        (js/games2.js:384 — „веднъж на ден" за балончето).
    //      · текстът на мама не се озовава НИКЪДЕ в паметта — проверено с
    //        обхождане на целия склад след натискането.
    //    Тоест `писа: true` в уреда идва от ЧУЖД запис, не от този текст.
    //    Тук изчистването на полето Е ПРОДУКТЪТ: листът се къса и си отива.
    //    „Поправка" по желязното правило би оставила яда на мама на екрана и
    //    би намекнала, че някъде е запазен — точно обратното на обещанието
    //    в заглавието („никой никога няма да го прочете“).
    //    ПЪТ НАЗАД: няма какво да се връща — нищо не е сменяно.
