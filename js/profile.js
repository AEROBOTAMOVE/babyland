...
(function () {
  ...
  function провериНиво(л) {
    ...
    if (typeof видяно === 'number' && л.n > видяно) {
      ...
      // 🔴🔴 25.08 (dev/lazhliv_uspeh.js): отговорът се хвърляше — стикерът не
      //    се записваше, празникът се правеше, а редът отдолу отбелязваше нивото
      //    за ВИДЯНО. Тоест стикерът се губеше ЗАВИНАГИ: следващия път условието
      //    `л.n > видяно` вече не е вярно и наградата няма как да се върне.
      //    Не мине ли записът, нивото остава „невидяно" — празникът ще дойде пак,
      //    когато има място. `save` вече казва истината (profile.js:71 → записПадна).
      if (!save('bl_goal_stickers', ст)) return;
  ...
  function провериНагради(st) {
    ...
    st.chosen.forEach(c => {
      ...
      if (p.готова) {
        ...
        // 🔴 25.08 (dev/lazhliv_uspeh.js): стикерът се записваше „на сляпо" и
        //    целта се обявяваше за постигната. При пълна памет стикерът го няма,
        //    а `st.done` вече казва „взето" → наградата се губи. Не мине ли
        //    записът, целта остава незавършена и ще се хване пак.
        if (!save('bl_goal_stickers', ст)) { delete st.done[c.id]; return; }
        има = деф;
      }
    });
    // и празникът е само ако завършването е ЗАПИСАНО — иначе при следващото
    // отваряне същата цел „се постига" пак и стикерът ѝ се удвоява.
    if (има) {
      if (!save('bl_goals', st)) return st;
      fx().confetti(); fx().cheer(има.e + ' Цел постигната: „' + има.n + '“!');
    }
    return st;
  }
  ...
  function входРегистрация(root) {
    ...
    създай.addEventListener('click', () => {
      ...
      // 🔴 25.08 (dev/lazhliv_uspeh.js): отговорът се хвърляше. При пълна памет
      //    или в частен прозорец падаха конфети, приложението казваше „Добре
      //    дошла, Ани!", `рисувайВ(root)` пре-рисуваше екрана — и написаното име
      //    изчезваше заедно с формата, а профил нямаше. Сега формата остава на
      //    място с името вътре. `save` вече казва истината (profile.js:71).
      if (!save('bl_mama', мама)) {
        греш.textContent = 'Не можах да създам профила — името ти стои в полето. Освободи малко място и натисни пак. 💛';
        return;
      }
      греш.textContent = '';
    ...
    файл.addEventListener('change', () => {
      ...
      rd.onload = async () => {
        try {
          ...
          // 🔴🔴 05.09 — същата дупка като в js/rooms2.js, същият лек.
          //   Вносът филтрираше ИМЕНАТА на ключовете и приемаше всяка стойност.
          //   Изразът е тесен: само отварящ таг с ЛАТИНСКА буква, „javascript:"
          //   и обработчик on…= . Български текст с „<" минава непокътнат.
          ...
          if (отхвърлени) {
            вход.appendChild(el('p', 'prof-warn', '<strong>' + отхвърлени +
              (отхвърлени === 1 ? ' нещо не влезе' : ' неща не влязоха') +
              '.</strong> В този файл има записи, които не приличат на нищо, писано от приложението. Останалото влезе.'));
          }
          if (недостиг) {
  ...
  function профил(root) {
    ...
    // 🤰 15.09 (E2E „бременна“, B7): „Още едно съкровище?“ се предлагаше и на бременна, чието
    //   първо бебе още не е родено — покана за второ дете със свои ваксини и възраст. Сега само
    //   когато първото наистина е родено (рождена дата в миналото, не режим „чакаме“).
    } else if (бебе.name && бебе.birth && бебе.sex !== 'wait' && new Date(бебе.birth).getTime() <= Date.now()) {
    ...
    root.appendChild(el('p', 'prof-legal', '🔒 Всичко тук е само на този телефон. Смяна на телефон: свали копието тук, а на новия го качи от същото място, горе в „💾 Данните ми“.'));
  }
