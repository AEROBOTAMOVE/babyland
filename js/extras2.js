...
(function () {
  ...
  function registryCard() {
    ...
    ['Столче за кола', 'Бодита р.62', 'Пелени NB', 'Гризалка', 'Мека книжка', 'Одеялце'].forEach(p => {
    ...
    function draw() {
      ...
      items.forEach((it, i) => {
        ...
        r.innerHTML = `<span class="jr-check">${it.done ? '✔' : ''}</span> <span class="rg-t">${esc(it.t)}</span><span class="nt-del" role="button">🗑</span>`;
        r.addEventListener('click', (e) => {
          if (e.target.classList.contains('nt-del')) { items.splice(i, 1); save('bl_registry', items); draw(); return; }
    ...
    addB.addEventListener('click', () => { const v = inp.value.trim(); if (!v) return; items.push({ t: v, done: false }); save('bl_registry', items); inp.value = ''; draw(); });
    ...
    shareB.addEventListener('click', async () => {
      ...
      try { await navigator.clipboard.writeText(text); fx().cheer('Копирано! Прати го на когото искаш 📤'); } catch (e) {}
    });
    c.appendChild(row); c.appendChild(presets); c.appendChild(list); c.appendChild(shareB);
    draw();
    return c;
  }
  ...
  function photoCard() {
    ...
    file.addEventListener('change', () => {
      ...
      img.onload = () => {
        ...
        if (!save('bl_photos', photos)) { fx().cheer('Паметта се напълни — изтрий стара снимка. 😕'); delete photos[target]; }
        else save('bl_photo_day', today()); // брои се в дневната разходка
    ...
    function view(m) {
      ...
      ov.innerHTML = `<div class="pho-viewbox"><img src="${esc(photos[m])}"><p>${esc(baby.name) || 'Бебето'} · ${m === 0 ? 'при раждането' : m + ' месеца'}</p>
      ...
      ov.querySelector('#phoDel').onclick = () => { delete photos[m]; save('bl_photos', photos); ov.hidden = true; draw(); };
  ...
  function bodyCard() {
    ...
    [['🥀', 'тежко'], ['🌱', 'средно'], ['🌸', 'добре']].forEach(([e, v]) => {
      ...
      b.addEventListener('click', () => {
        ...
        msg.textContent = v === 'тежко' ? 'Чуто. Днес — минимум задачи, максимум милост. 🤗' : v === 'средно' ? 'Стъпка по стъпка. Тялото ти направи чудо. 🌱' : 'Прекрасно! Отбележи си какво помага. 🌸';
      });
      row.appendChild(b);
    });
    ...
    note.addEventListener('input', () => { data[t] = data[t] || { v: '' }; data[t].n = note.value; save('bl_body', data); });
    ...
    рисувайМинали();
    c.appendChild(row); c.appendChild(msg); c.appendChild(note); c.appendChild(минали);
    return c;
  }
  ...
  function shopConfigCard() {
    ...
    inp.addEventListener('change', () => save('bl_shop_url', inp.value.trim()));
    c.appendChild(inp);
