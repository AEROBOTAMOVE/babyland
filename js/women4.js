...
(function () {
  ...
  function работаCard() {
    ...
    дата.addEventListener('change', () => {
      // 🔴 25.08 (при обхода с dev/pylna_pamet.js, видяно с очи — уредът натиска
      //    само бутони и полета с `input`, до `change` не стига): върдиктът на
      //    записа се хвърляше и отдолу пишеше „Записах датата. ✓" дори когато
      //    първият работен ден не е стигнал до паметта.
      st.date = дата.value;
      if (запази('date') === false) { знРаб.кажи(НЕ_СЕ_ПОБРА, 7000); fx().buzz(6); return; }
    ...
    const рисувайМои = () => {
      ...
      st.slice().reverse().forEach((x, ri) => {
        ...
        трий.addEventListener('click', () => {
          ...
          // 🔴 25.08 (dev/lazhliv_uspeh.js): `рисувайМои()` чете НАНОВО от паметта
          //    и сам връща реда на екрана — но редът отдолу вече беше казал
          //    „Махнато." При пълна памет мама вижда изречението си пак ПЛЮС
          //    надпис, че е махнато, и бутон „↩️ Върни последното", който би го
          //    удвоил. Обявяваме махането само ако е станало.
          if (!save('bl_wm_replies', st)) {
            последнаР = null;                                  // няма какво да се връща
            рисувайМои();
            знР.кажи('Не можах да го махна — паметта на телефона е пълна. Редът си остава при теб.', 7000);
            fx().buzz(6);
            return;
          }
          рисувайМои(); fx().buzz(8);
