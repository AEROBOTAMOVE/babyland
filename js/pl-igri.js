...
(function () {
  ...
  // ── 🖼️ плюшена фигурка за всяка стъпка (реф. 12: всяка от трите стъпки носи СНИМКА) ──
  // Нови картинки НЕ се генерират (кредитите са на собственика) — фигурките са от готовите
  // спрайтове img/art/ico-a…e.webp. Думите отдолу са ИЗВАДЕНИ от текста на 35-те занимания
  // в data.js:158–194, не са измислени: напр. „Скрий лице зад кърпа“ → зайчето под одеялцето,
  // „Тенджера + дървена лъжица“ → купичката, „Сложи бебето по коремче“ → бебето по коремче.
  // Първото съвпадение печели, затова редът е от по-точното към по-общото.
  const ФИГУРИ = [
    [/скрий|скрит|къде е|ку-ку|намери я|лов на|съкровищ|изненад/i, 'ico-e', 2, 3],   // зайче под одеялце
    [/кърпа|шалче|платче|чорап|одеял|пелен|дрешк|пране|сортира“ чорапи/i, 'ico-d', 3, 3], // одеялце
    [/кубче|кула|чашк|строй|събаря|сортир|размер|цвят или размер/i, 'ico-b', 2, 0],  // кубчета
    [/лъжиц|тенджер|купа|купичк|макарон|прибор|капачк|кухн|готвач|сипва|бърка|гъба/i, 'ico-a', 2, 0], // купичка
    [/рисув|пастел|драск|творб|изкуство|цветове|шарки|черно-бяло/i, 'ico-b', 2, 2],  // палитра
    [/лист|хартия|вестник|къса|мачка|шумол|амбалаж/i, 'ico-b', 0, 3],                // молив
    [/песен|песничк|музика|танц|ритъм|барабан|оркестър|пляс|дрънка|звук/i, 'ico-d', 3, 1], // дрънкалка
    [/говор|казва|кажи|дума|повтаря|разговор|коментира|общуван|език|посоч|„аа“/i, 'ico-d', 1, 1], // балонче-реч
    [/огледал/i, 'ico-d', 3, 0],                                                     // рамка сърце
    [/коремче|гръбче|легни|постел|заляга/i, 'ico-d', 0, 1],                          // бебе по коремче
    [/пълзи|пълзет|тунел|ходет|мечк|скача|зайц|котк|стъпк|крачет|колело|баланс/i, 'ico-d', 1, 0], // пълзящо бебе
    [/виж|гледа|покажи|оглежда|очи|блести|контрастн|търсите/i, 'ico-c', 3, 1],        // лупа
    [/играчк|мече/i, 'ico-d', 0, 3],                                                 // мече
    [/кутия|кошниц|кошничк|сандък|вади/i, 'ico-a', 1, 3],                            // сандъче
    [/бутилк|шише|лед/i, 'ico-a', 1, 0],                                             // шише
    [/възглавниц|мост|пътечк/i, 'ico-e', 0, 0],                                      // постелка
    [/гушн|прегърн|сърце|обич|любов|връзка|докосван|сетива/i, 'ico-c', 2, 1],         // две сърца
    [/разходк|количк/i, 'ico-d', 2, 1],                                              // количка
    [/мин\b|минут|време/i, 'ico-e', 3, 0],                                           // пясъчен часовник
  ];
  // когато нито една дума не съвпадне — фигурка по МЯСТОТО на стъпката, за да не излязат
  // три еднакви: приготви (мече) → мисъл (облаче) → заедно (две сърца)
  const ФИГ_РЕЗЕРВА = [['ico-d', 0, 3], ['ico-c', 2, 0], ['ico-c', 2, 1]];
  function фигура(текст, i, предишна) {
    const н = ФИГУРИ.find(р => р[0].test(текст));
    let ф = н ? [н[1], н[2], н[3]] : ФИГ_РЕЗЕРВА[i % 3];
    if (предишна && ф.join() === предишна.join()) ф = ФИГ_РЕЗЕРВА[i % 3];   // две еднакви подред — не
    return ф;
  }
  ...
  // ── надписите на чекмеджетата на ЕДИН ред, както в реф. 1 („Кутия за спомени“ се пречупваше) ──
  // 🪤 мерено 22.09 (находка на проверителя): при 390px за текста остават 102px, а „Кутия за
  //   спомени“ иска 124px при 13px Georgia — на този телефон един ред просто НЕ се побира
  //   четимо (виж сметката в css/pl-igri.css). Не гадаем: мерим с ИСТИНСКИЯ шрифт (на Android
  //   серифът е Noto Serif, по-широк) и слагаме на ДВЕТЕ чекмеджета ЕДНАКВА големина, за да не
  //   са разнобойни. Подът е 88% — ако и там не влиза (390px), НЕ насилваме: оставяме го да се
  //   пречупи, както е сега. На по-широк телефон влиза и двете стават на един ред, както в реф. 1.
  //   Връща false, ако още няма размер (мерим пак).
  function единРед(списък, дял) {
    const живи = списък.filter(е => е && е.isConnected);
    if (!живи.length) return true;
    const к = платно || (платно = document.createElement('canvas').getContext('2d'));
    const дъно = [];
    let най = Infinity;
    for (const е of живи) {
      if (е.style.fontSize || е.style.whiteSpace) { е.style.fontSize = ''; е.style.whiteSpace = ''; }
      const cs = getComputedStyle(е), ш = е.clientWidth;
      if (!(ш > 0)) return false;
      const т = (е.textContent || '').trim();
      if (!т) continue;
      const мери = px => { к.font = cs.fontStyle + ' ' + cs.fontWeight + ' ' + px + 'px ' + cs.fontFamily; return к.measureText(т).width; };
      const от = parseFloat(cs.fontSize), пода = от * (дял || 0.78);
      let px = от;
      while (px > пода && мери(px) > ш - 1) px -= 0.5;
      if (мери(px) > ш - 1) return true;               // не влиза дори на пода → оставяме го да се пречупва
      дъно.push([е, от]); най = Math.min(най, px);
    }
    if (!isFinite(най)) return true;
    дъно.forEach(([е, от]) => { е.style.whiteSpace = 'nowrap'; if (най !== от) е.style.fontSize = най + 'px'; });
    return true;
  }
  ...
  //   плавност. Само scrollTop — DOM не се пипа. Пръст/колелце го спират.
  // 🪤 22.09, ВТОРИ КРЪГ (находка на проверителя, ver_igri_skok.js): „Спомени“ замръзваше на
  //   1598 px в 1 от 5 опита. Виновно беше СЛЯПОТО броене „3 опита / 4 с“: при бърз допир,
  //   докато стаята още дорисува карти, трите корекции се изхабяваха от самия РАСТЕЖ на
  //   съдържанието (мерено: скрол 1697 → 3012 → 3416 → 3667) и следващото разместване нямаше
  //   кой да хване. Сега напразен опит се брои САМО когато целта НЕ е мръднала между две
  //   корекции (значи не съдържанието ни бута, а нещо друго ни държи) — растежът връща брояча
  //   на нула. Спираме, когато целта е горе И НЕ е мърдала 8 проверки подред (≈1.2 с покой),
  //   а не на първото попадение: карта може да се дорисува и след него.
  //   Тавани: 8 напразни корекции или 8 с общо.
  // 🪤 23.09, ТРЕТИ КРЪГ — инструментирано (ag2_igri_diag_b.js: закърпен scrollIntoView + проба
  //   на 100ms). Две неща, които не се виждат без уред:
  //   1) Стаята се пренарежда ДЪЛГО: височината ѝ скача 4132 → 5462 → 6019 → 5828 → 5732 → 6219
  //      чак до t=5.6 с след клика. Единствената поправка на „Играем заедно“ падна на t=5108 —
  //      под стария таван от 4000ms тя изобщо нямаше да се случи. Таванът е 10 с.
  //   2) ИСТИНСКАТА причина за замръзването: `if (!е) { край(); return; }`. Докато стаята
  //      прерисува, целевата глава/карта за миг я НЯМА в DOM — и цикълът умираше ЗАВИНАГИ.
  //      Точно затова „Спомени“ спираше на 1598px, а „Кутия за спомени“ на 2773px: един
  //      пропуснат кадър убиваше пазача. Сега липсваща цел значи „още се рисува — чакай“.
  //   Край: целта е горе, не е мърдала И височината на стаята не се е сменила ~1.5 с; или
  //   10 напразни поправки (целта не може да се качи — напр. е последната); или 10 с.
  //   Пръст/колелце спират всичко веднага.
  function докарай(стая, цел) {
    let пред = -1, тихо = 0, напразно = 0, спокойни = 0, целПред = null, висПред = -1, спри = false;
    const т0 = Date.now();
    ...
    const стъпка = () => {
      if (спри || !стая.isConnected || Date.now() - т0 > 10000) { край(); return; }
      const вис = стая.scrollHeight;
      const мирна = вис === висПред; висПред = вис;
      const е = цел();
      if (!е) { тихо = 0; спокойни = 0; setTimeout(стъпка, 150); return; }   // прерисува се — чакаме, НЕ се отказваме
      ...
      if (тихо >= 2) {
        ...
        const абс = ст + разл;                       // мястото на целта В СЪДЪРЖАНИЕТО, не на екрана
        const мръдна = целПред === null || Math.abs(абс - целПред) > 2;
        целПред = абс;
        if (Math.abs(разл) <= 24) {
          спокойни = (мирна && !мръдна) ? спокойни + 1 : 0;
          if (спокойни >= 10) { край(); return; }    // горе е, не мърда И стаята не расте ≈1.5 с
        } else {
          напразно = (мирна && !мръдна) ? напразно + 1 : 0;   // растеж ≠ напразен опит
          спокойни = 0;
          if (напразно >= 10) { край(); return; }    // не можем да я качим (напр. е последната) — пускаме
          тихо = 0;
          // 🪤 мерено (ag2_igri_spom_b.js): scrollIntoView понякога НЕ докарва целта горе —
          //   той скролва „най-близкия скролируем прародител“ и се разправя с котвата на скрола.
          //   Прякото писане на scrollTop е еднозначно: поисках 3675 → получих 3675 → d = 0.
          //   scrollIntoView остава само за резерва (ако целта е в друг, вложен скрол).
          стая.scrollTop = ст + разл;
          if (напразно >= 3 && напразно % 3 === 0) е.scrollIntoView({ block: 'start', behavior: 'auto' });
        }
      }
  ...
  function сложи() {
    ...
    document.querySelectorAll('#roRoom').forEach(стая => {
      ...
      if (е.getAttribute('data-pl-fit') !== ключ && е.offsetWidth) {
        const заг = е.querySelector('.pl-ig-title');
        const ок1 = заг ? побери(заг) : true;
        const ок2 = единРед([...е.querySelectorAll('.pl-ig-drawer span')], 0.88);
        if (ок1 && ок2) е.setAttribute('data-pl-fit', ключ);
      }
    });
  ...
  function отвориЛист(игра, бутон) {
    ...
    // реф. 12: всяка стъпка е рамка с фигурка, а номерът е кръгче върху горния ѝ ъгъл
    let пред = null;
    с.querySelector('.pl-ig-steps').innerHTML = стъпки(игра.x).map((x, i) => {
      const ф = фигура(x, i, пред); пред = ф;
      return '<li class="st-' + (i + 1) + '">' +
        '<span class="pl-ig-sfig">' +
          '<i class="pl-ig-i" aria-hidden="true" style="' + ико(ф[0], ф[1], ф[2]) + '"></i>' +
          '<b aria-hidden="true">' + (i + 1) + '</b>' +
        '</span>' +
        '<span class="pl-ig-stx">' + esc(x) + '</span></li>';
    }).join('');
  ...
  function върви() {
    ...
    // завъртане на телефона → мерките с canvas.measureText са за старата ширина; махаме
    // маркера и мерим пак (само при ДРУГА ширина — иначе клавиатурата на Android би ни въртяла)
    let ш = innerWidth, изчакване = null;
    addEventListener('resize', () => {
      if (innerWidth === ш) return;
      ш = innerWidth;
      clearTimeout(изчакване);
      изчакване = setTimeout(() => {
        document.querySelectorAll('#roRoom > .pl-ig').forEach(е => е.removeAttribute('data-pl-fit'));
        try { сложи(); } catch (e) {}
        if (лист && !лист.hidden) побери(лист.querySelector('#plIgT'));
      }, 220);
    }, { passive: true });
    сложи();
  }
