...
(function () {
  ...
  // 🪤 мерено 22.09 (находка на проверителя): при 390px за текста остават ~100px, а „Кутия за
  //   спомени“ е 16 знака Georgia 15px. Не гадаем колко да смалим — мерим с ИСТИНСКИЯ шрифт
  //   (на Android серифът е Noto Serif, по-широк) и слагаме на ДВЕТЕ чекмеджета ЕДНАКВА големина,
  //   за да не изглеждат разнобойни. Ако и на пода (78%) не влиза — не насилваме: махаме nowrap
  //   и го оставяме да се пречупи, както беше. Връща false, ако още няма размер (мерим пак).
  ...
  function докарай(стая, цел) {
    let пред = -1, тихо = 0, напразно = 0, покой = 0, целПред = null, спри = false;
    const т0 = Date.now();
    ...
    const стъпка = () => {
      const е = цел();
      if (спри || !е || !стая.isConnected || Date.now() - т0 > 8000) { край(); return; }
      ...
      if (тихо >= 2) {
        ...
        целПред = абс;
        if (Math.abs(разл) <= 24) {
          покой = мръдна ? 0 : покой + 1;
          if (покой >= 8) { край(); return; }        // горе е И не е мърдала 8 проверки подред
        } else {
          напразно = мръдна ? 0 : напразно + 1;      // расте съдържанието ≠ напразен опит
          покой = 0;
          if (напразно >= 8) { край(); return; }     // не можем да я качим (напр. е последната) — пускаме
          тихо = 0;
  ...
  function сложи() {
    ...
    document.querySelectorAll('#roRoom').forEach(стая => {
      ...
      if (е.getAttribute('data-pl-fit') !== ключ && е.offsetWidth) {
        ...
        const ок2 = единРед([...е.querySelectorAll('.pl-ig-drawer span')], 0.78);
