...
(function () {
  ...
  function записвачка(име, наИзход) {
    ...
    function рисувай() {
      ...
      if (има && !тече) {
        ...
        // 🔴 25.08 (обход „Игри и Лаборатория"): ЕДНО докосване триеше гласа на
        //    мама — записан веднъж, докато е имала сили да пее — без въпрос и
        //    без връщане. Бутонът беше и невидим за четеца (само `title`, без
        //    име). Това е единственият запис в приложението, който НЕ може да
        //    се възстанови от нищо. Правило 3 на къщата: пътят назад ПРЕДИ
        //    действието. Две стъпки, както „⏹️ Спри опита" и „✕" в лаб.
        //    ПЪТ НАЗАД: махни `сигурна`/`часовник` и остави само тялото на else.
        const трий = el('button', 'jr-chip lr-extra', '🗑️'); трий.type = 'button';
        трий.title = 'Изтрий записа';
        трий.setAttribute('aria-label', 'Изтрий записа с моя глас за „' + име + '“');
        трий.style.minWidth = '44px'; трий.style.minHeight = '44px';
        let сигурна = false, часовник = null;
        трий.addEventListener('click', () => {
          if (!сигурна) {
            сигурна = true;
            трий.textContent = '🗑️ Да изтрия ли гласа ти?';
            трий.classList.add('on');
            трий.setAttribute('aria-label', 'Натисни пак, за да изтриеш записа с гласа си');
            бел.textContent = 'Записът си отива завинаги — няма как да го върна. Натисни пак, ако си сигурна.';
            fx().buzz(8);
            clearTimeout(часовник);
            часовник = setTimeout(() => {
              сигурна = false; трий.textContent = '🗑️'; трий.classList.remove('on');
              трий.setAttribute('aria-label', 'Изтрий записа с моя глас за „' + име + '“');
          ...
          clearTimeout(часовник);
          const з = записите(); delete з[име]; save(ЗАПИСИ, з); fx().buzz(6); рисувай();
          if (наИзход) наИзход();          // чипът на песничката губи своето 🎤
        });
  ...
  function картаПесни() {
    ...
    Object.keys(ОЩЕ_ПЕСНИ).forEach(име => {
      ...
      b.addEventListener('click', () => {
        // 🔴 25.08 (обход „Игри и Лаборатория", ИЗМЕРЕНО): смяната на песничка
        //    НЕ спираше робота. Натиснеш „Изпей я вместо мен", избереш друга —
        //    и на екрана пише едно, а гласът чете друго. Сестринската карта в
        //    extras2.js (приспивните) прави точно това от 11.08; тук го нямаше.
        ...
        // 🔴 25.08 (обход „Игри и Лаборатория", ИЗМЕРЕНО живо: бутонът беше
        //    един от двата НЕМИ в цялата стая): натискаш „Изпей я вместо мен"
        //    в 3 през нощта — роботът почва да чете, а по бутона НИЩО не се
        //    променя и НЯМА как да го спреш. Второто натискане само рестартира
        //    от началото. Ако телефонът няма български глас (често), не става
        //    абсолютно нищо и никой не ти го казва.
        //    Точно това беше поправено на 11.08 в СЪСЕДНАТА карта (extras2.js,
        //    приспивните) — близнакът тук остана. Един похват и на двете места:
        //    ключе, което казва в кое положение е, и честна дума при липса.
        //    ПЪТ НАЗАД: върни трите реда „speechSynthesis.cancel/speak" без
        //    покой()/onend/onerror.
        const ПОКОЙ = '🔊 Изпей я вместо мен';
        const робот = el('button', 'jr-chip', ПОКОЙ); робот.type = 'button';
        const покой = () => { робот.textContent = ПОКОЙ; };
        const немоглИ = () => { покой(); fx().cheer('Този телефон не може да пее на глас — но думите са тук, за теб 💜'); };
        робот.addEventListener('click', () => {
          try {
            ...
            u.onend = покой; u.onerror = немоглИ;
            speechSynthesis.speak(u);
            робот.textContent = '⏹️ Спри пеенето';
            // някои телефони нито почват, нито гърмят — ключето да не остане вдигнато
            setTimeout(() => { try { if (!speechSynthesis.speaking && !speechSynthesis.pending) покой(); } catch (e) { покой(); } }, 1200);
          } catch (e) { немоглИ(); }
        });
  ...
  function пуснБалонче() {
    ...
    // 🔴 25.08 (обход „Игри и Лаборатория"): `к.querySelector('.jr-btn').click()`
    //    беше БЕЗ пазач — липсва ли бутонът, тук гърми TypeError вътре в
    //    слушателя на „🎈 30 секунди" и лентата просто замръзва.
    const натисни = к => { const б = к && к.querySelector('.jr-btn'); if (!б) return false; б.click(); return true; };
    if (натисни(намери())) return;
    if (!window.MamaHelper) return;
    MamaHelper.open('Дневник на мама');
    // 🔴 25.08 (ИЗМЕРЕНО в кода на hero.js): чакахме РОВНО 500 мс. Но hero.js
    //    обвива MamaHelper.open и първо ЛЕТИ до къщичката: 700 мс полет + 420 мс
    //    повдигане ≈ 1120 мс, преди стаята изобщо да почне да се строи. Тоест
    //    щом лентата е на началния екран (а тя стои 22 секунди и преживява
    //    затварянето на стаята), „🎈 30 секунди" беше МЪРТЪВ БУТОН: мама
    //    натиска, стаята се отваря и играта не тръгва. Никой не се оплаква от
    //    игра, която не тръгва — просто я затваря.
    //    Сега не гадаем колко трае анимацията: чакаме картата да се появи.
    //    ПЪТ НАЗАД: върни едно `setTimeout(..., 500)`.
    let опити = 0;
    (function чакай() {
      if (натисни(намери())) return;
      if (++опити > 20) {                       // ~3 секунди — после честна дума
        fx().cheer('Балончето е в „Дневник на мама“, картата „Пукни балончето 🎈“. 💜');
        return;
      }
      setTimeout(чакай, 150);
    })();
  }
