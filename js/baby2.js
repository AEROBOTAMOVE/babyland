...
(function () {
  ...
  function форма(готово) {
    ...
    добави.addEventListener('click', () => {
      ...
      //    падаха конфети, приложението казваше „Добре дошло, …!", `готово()`
      //    сваляше формата и името/датата изчезваха от екрана, без нищо да е
      //    записано. `save` вече вика BL_ZAPIS_PADNA (baby2.js:17); тук остава
      //    само формата да НЕ се маха и празникът да НЕ се прави.
      if (!save('bl_baby2', { name: n.slice(0, 24), birth: дата.value || '', sex: избран, d: new Date().toISOString().slice(0, 10) })) {
        греш.textContent = 'Не можах да го запазя — написаното ти остава тук, в полетата. 💛';
        return;
      }
      греш.textContent = '';
