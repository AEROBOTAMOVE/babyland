...
(function () {
  ...
  const ПРЕДМЕТ = /играчк|мече|мечо|кукл|плюшен|бордов|обиколник|позиционер|кокон|гнезд|пашкул|подложк|дунапрен|възглавка/i;
  const МЯСТО_ЗА_СЪН = /легл|креват|кошар|матрак|кошче/i;
  function isBedRisk(t) {
    const s = String(t || '');
    return ПОЗА_ЛЕГЛО.test(s) || (ПРЕДМЕТ.test(s) && МЯСТО_ЗА_СЪН.test(s));
  }
