...
(function () {
  ...
  function qrCard() {
    ...
    къмSOS.addEventListener('click', () => {
      ...
      // 🔴 12.08 (ревизия): коментарът обещаваше „най-много 4 сек“ за отварянето,
      //    а кодът пускаше СЪЩИЯ праг 1500 и за двете фази — тоест ако оверлеят
      //    изобщо не се появи (друг модул, друг бутон), часовникът тиктакаше
      //    2.5 пъти в секунда цели 10 МИНУТИ в телефона на мама. И всяко ново
      //    натискане пускаше ОЩЕ ЕДИН. Сега: 4 сек за отварянето, 10 мин за
      //    затварянето, и не повече от един часовник наведнъж.
      //    ПЪТ НАЗАД: върни `let видян = false, тик = 0;` и единия праг 1500.
      clearInterval(къмSOS._пази);
      let видян = false, тик = 0;
      къмSOS._пази = setInterval(() => {
        const ов = document.getElementById('sosOverlay');
        const отворен = !!ов && !ов.hidden;
        if (отворен) видян = true;
        тик++;
        const край = !document.body.contains(кутия)          // картата си отиде
          || (!видян && тик >= 10)                            // 4 сек и оверлей няма
          || (видян && !отворен)                              // отворил се е и се е затворил
          || тик >= 1500;                                     // таван 10 мин
        if (!край) return;
        clearInterval(къмSOS._пази); къмSOS._пази = null;
