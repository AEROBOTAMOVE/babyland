...
(function () {
  ...
  const BADGES = [
    ...
    // 🔴 19.08 (dev/broyachi.js, ИЗМЕРЕНО живо): този медальон и `foods25`
    //    имаха ДУМА ПО ДУМА едно и също условие и един и същ надпис —
    //    „25 опитани храни“. При 25-ата храна светваха ЕДНОВРЕМЕННО: два
    //    празника един след друг и после два еднакви реда на рафта. Мама е
    //    направила ЕДНО нещо, а приложението ѝ казва, че е направила две —
    //    и на рафта „40 медальончета“ едно място беше копие.
    //    Стълбата, която каталогът вече ползва другаде (7/30 разходки,
    //    3/5 опита, 7/30 записа), тук беше 10 → 25 → 25. Третото стъпало
    //    става 40.
    //    ⚠️ Ид-то НЕ се сменя и медальонът НЕ се маха от каталога — иначе
    //    майка, която вече го има, щеше да го загуби от рафта, а правилото
    //    на къщата е „веднъж взето — завинаги“ (badges2.js). Който го е
    //    взел, си го задържа; надписът под него става следващата ѝ цел.
    //    ПЪТ НАЗАД: върни `t: '25 опитани храни'` и `>= 25`.
    { id: 'tried25', e: '🍽️', n: 'Гурме двойка', t: '40 опитани храни', ok: () => Object.keys(load('bl_tried', {})).length >= 40 },
  ...
  // ═══ ЕДИН ИЗТОЧНИК НА ИСТИНАТА: „кои са НЕЙНИТЕ медальончета“ ═══
  // 🔴 19.08 (dev/broyachi.js, ИЗМЕРЕНО): три места смятаха това поотделно —
  //    рафтът тук, страницата в Годишника (badges2.js) и надписът „N от 40“
  //    в профила. Първите две пресичат паметта с КАТАЛОГА; третото брои голите
  //    ключове в bl_badges. Щом в паметта попадне ид, което го няма в каталога
  //    (внесено резервно копие от друга версия), профилът пише „2 от 40“, а
  //    светнал е ЕДИН. ИЗМЕРЕНО в пясъчник: 2 срещу 1.
  //    Формулата живее ТУК, на едно място. Рафтът отдолу и Годишникът
  //    (badges2.js) вече питат нея. profile.js:686 още брои сам — там лекът е
  //    един ред: `Object.keys(взети).length` → `BL_BADGES.броени()`. Файлът е
  //    чужд, затова не го пипам, но вратата е отворена и не иска нищо ново.
  //    Връща [{ b, кога }] в реда на каталога — подреждането е на викащия,
  //    защото Годишникът иска най-старото първо, а рафтът — най-новото.
  function моитеМедальони() {
    const got = load('bl_badges', {});
    return BADGES.filter(b => got[b.id]).map(b => ({ b: b, кога: Number(got[b.id]) || 0 }));
  }
  // профилът показва рафта; `мои`/`броени` са за всеки, който иска ЧИСЛОТО
  window.BL_BADGES = { sweep: sweepBadges, list: BADGES, мои: моитеМедальони,
                       броени: () => моитеМедальони().length };

  function badgesCard() {
    const c = card('Медальончета 🏅 <span class="jr-sub">малките победи, събрани завинаги</span>');
    sweepBadges();                      // първо метенето, после четенето
    ...
    // 19.08: списъкът идва от моитеМедальони() — същата формула, която ползват
    // Годишникът и профилът. Тук само подреждаме.
    const мои = моитеМедальони().sort((x, y) => y.кога - x.кога);
    мои.forEach(({ b, кога: печат }) => {
      ...
      let кога = '';
      try { if (печат > 0) кога = ' · дойде ' + new Date(печат).toLocaleDateString('bg-BG'); } catch (e) {}
  ...
  function lenHeadCard() {
    ...
    btn.addEventListener('click', () => {
      ...
      без.push(нов);
      // 🔴 25.08 (dev/lazhliv_uspeh.js): отговорът се хвърляше и „✔ Записано за
      //    днес" излизаше и при пълна памет. `save` тук вече вика BL_ZAPIS_PADNA
      //    (extras.js:13) — остава само да НЕ лъжем: без надпис, без бутон „Отмени"
      //    за запис, който го няма. Числото горе (перцентилът) си остава на екрана.
      const записан = save(ключ, без.slice(-24));
      if (!записан) return;
      ...
      отмени.addEventListener('click', () => {
        ...
        // Същото и назад: ако връщането не се запише, не казваме „Върнах го".
        if (!save(ключ, сега.slice(-24))) return;
        действие.innerHTML = '';
