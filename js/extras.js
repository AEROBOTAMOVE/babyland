...
(function () {
  ...
  function shareMemCard() {
    ...
    bb.addEventListener('click', () => { if (ti.value.trim()) shareCard({ emoji: '🎈', title: ti.value.trim(), sub: 'Baby Land моменти' }); });
  ...
  function breathCard() {
    const c = card('SOS дъх 🌬️ <span class="jr-sub">60 секунди спокойствие, когато всичко е много</span>');
  ...
  const BADGES = [
    ...
    { id: 'baby22', e: '🍼', n: 'Двойна прегръдка', t: 'Добавено второ съкровище', ok: () => !!(load('bl_baby2', {}).name || '').trim() },
  ...
  function lenHeadCard() {
    ...
    btn.addEventListener('click', () => {
      ...
      if (!baby.sex || (baby.sex !== 'boy' && baby.sex !== 'girl')) {
        out.innerHTML = 'Кажи ми първо момче ли е, или момиче — кривите на СЗО са различни за двете. Изборът е в картата „Профилът на бебето“, в стая „Моето бебе“. 💜';
        box.innerHTML = '';
        return;
      }
  ...
  function pinAsk(title, onOk) {
    ...
    //    ТРИ ключа (дневниче, изповеди, „греховете“) — те си отиват с нея,
    //    иначе изходът щеше да е задна врата за всеки, взел телефона.
    //    Останалото в приложението остава непокътнато.
    ...
    forgot.onclick = () => {
      ...
      exitBox.innerHTML = '<p>Ключалката може да се махне — но тя пази дневничето, ' +
        'изповедите и „греховете“. <strong>Те си отиват с нея.</strong> ' +
      ...
      да.onclick = () => {
        ['bl_pin_h', 'bl_pin', 'bl_wm_diary', 'bl_wm_confess', 'bl_wm_sins']
  ...
  function pinCard() {
    ...
    c.appendChild(el('p', 'jr-privacy',
      '⚠️ <strong>Забравиш ли го — никой не може да ти го върне</strong>, защото никъде го няма. Има само един път назад: след три опита се появява „Забравих ПИН-а“ и ключалката се маха — но заедно с дневничето, изповедите и „греховете“, които пази. Затова избери цифри, които няма да забравиш и след безсънна нощ.'));
    return c;
  }
