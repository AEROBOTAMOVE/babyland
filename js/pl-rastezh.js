...
(function () {
  ...
  // ── 🔬 ИСТИНСКАТА ширина на надпис: мери се с уреда, не се гади ──────────────────────────
  //    Дотук се смяташе „текст.length × 6.6“ и „датата е 28 px“ — оценки на око, заради които
  //    проверителят хвана изчезващи етикети. Сега питаме браузъра (getComputedTextLength) през
  //    скрития <svg class="pl-rz-meas"> в блока, с истинския шрифт и истинския размер.
  //    Мълчи ли уредът (шрифтът още се зарежда, блокът е display:none) — връщаме старата оценка
  //    и НЕ я запомняме, за да се премери наново, щом стане възможно.
  const кешШ = new Map();
  function ширТекст(с, текст, роля, оценка) {
    const ключ = роля + ' ' + текст;
    const п = кешШ.get(ключ);
    if (п != null) return п;
    let ш = 0;
    const т = с && с.querySelector('.pl-rz-meas [data-r="' + роля + '"]');
    if (т) {
      try { if (т.textContent !== текст) т.textContent = текст; ш = т.getComputedTextLength(); } catch (е) { ш = 0; }
    }
    if (!(ш > 0)) return оценка;
    ш = Math.ceil(ш);
    кешШ.set(ключ, ш);
    return ш;
  }
  // шрифтът се зарежда след първото рисуване → мерките се сменят: чистим кеша и рисуваме наново
  if (document.fonts && document.fonts.ready && document.fonts.ready.then) {
    document.fonts.ready.then(() => {
      кешШ.clear();
      document.querySelectorAll('section.pl-rz').forEach(с => { с._plSig = ''; });
      отложено();
    }).catch(() => {});
  }
  ...
  function графиката(данни, к, Ш, ид, избД, с) {
    const М = МЕРКИ[к], Н = ВИС;
    // Полетата са свити (ляво 30→27, дясно 22→12): страната на списъка порасна до 141 px и
    // графиката падна от 196 на ~189 px (на 390 px екран; под 380 px тя е на цял ред — 302 px).
    // Проверено, че числата на оста („120“ = най-широкото) и четирите дати пак се побират —
    // вече с МЕРЕНИ ширини, не с оценки.
    const пл = 27, пд = 12, пг = 34, пдн = 28;           // полета: ляво (числата), дясно, горе, долу (датите)
    // 🪤 МЕРЕНО (ag2_rz_f_rez2, ДОВЪРШЕНО И ПРОВЕРЕНО тук: wf2_rz_geom_predi/wf2_rz_geom_sled):
    //    при по-тясна графика „15.08“ отпадаше — краищата бяха празни (16 px отляво, 19 px
    //    отдясно), а четирите дати се блъскаха в средата. Полето отляво стига 11 px (точката е
    //    r=5), отдясно 15 px (последната точка е с ореол r=13). Мерено на живо при Ш=187:
    //    първата точка cx=38 → левият ѝ край е на 33 px, вътре в мрежата (тя почва на пл=27);
    //    последната cx=160 → ореолът свършва на 173, тоест 14 px от ръба на листа и 2 px преди
    //    края на мрежата (Ш−пд=175). Нито един надпис не излиза извън листа (текст_извън: []),
    //    а в реф.-случая се виждат и четирите дати. Така първата точка стои до оста, както в реф. 16.
    const x0 = пл + 11, x1 = Ш - пд - 15;               // точките не лепнат по ръбовете
    ...
    const етикети = [];
    // Един етикет = един опит „голям/обикновен“, а при сгъстени точки — втори опит СМАЛЕН
    // (10 px) и чак тогава се пропуска. Проверителят хвана изчезнало „7,2 кг“ при две мерения
    // през 3 дни; сега стойността си остава на графиката, само по-дребна.
    const опит = (i, роля) => {
      const п = т[i], голям = роля === 'vl-last', дребен = роля === 'vl-sm';
      const текст = число(к, п.v) + ' ' + М.ед;
      const шир = ширТекст(с, текст, роля, текст.length * (голям ? 8.2 : дребен ? 5.8 : 6.6));
      const вис = голям ? 14 : дребен ? 10 : 11;
      ...
      етикети.push('<text class="pl-rz-vl' + (голям ? ' last' : '') + (дребен ? ' sm' : '') + (i === иИзбр ? ' sel' : '') +
        '" x="' + f(x) + '" y="' + f(y) + '" text-anchor="' + котва + '">' + текст + '</text>');
      return true;
    };
    ред.forEach(i => { if (i === посл) опит(i, 'vl-last'); else if (!опит(i, 'vl')) опит(i, 'vl-sm'); });
    // датите долу: последната, първата, после отзад напред
    const заетиД = [];
    const редД = [посл].concat(посл > 0 ? [0] : []);
    for (let i = посл - 1; i > 0; i--) редД.push(i);
    редД.forEach(i => {
      const п = т[i], текстД = оска(п.d);
      const шир = ширТекст(с, текстД, 'xl', 28);      // МЕРЕНА, не гадана (преди: твърдо 28 px)
      let x = п.x, котва = 'middle', а = x - шир / 2, б = x + шир / 2;
      if (б > Ш - 2) { x = Ш - 2; котва = 'end'; а = x - шир; б = x; }
      if (а < 2) { x = 2; котва = 'start'; а = x; б = x + шир; }
      if (заетиД.some(з => !(б < з[0] - 1 || а > з[1] + 1))) return;
      заетиД.push([а, б]);
      етикети.push('<text class="pl-rz-xl" x="' + f(x) + '" y="' + (Н - 9) + '" text-anchor="' + котва + '">' + текстД + '</text>');
    });
  ...
  function строй() {
    ...
    с.innerHTML =
      ...
      '</div>' +
      // 🔬 УРЕДЪТ за ширината на надписите (виж ширТекст). Строи се ВЕДНЪЖ, заедно със скелета —
      //    не е отделна мутация; след първото мерене отговорите идват от кеша и той не се пипа.
      '<svg class="pl-rz-meas" aria-hidden="true" focusable="false">' +
        '<text class="pl-rz-vl" data-r="vl"></text><text class="pl-rz-vl sm" data-r="vl-sm"></text>' +
        '<text class="pl-rz-vl last" data-r="vl-last"></text><text class="pl-rz-xl" data-r="xl"></text></svg>';
  ...
  function обнови(с) {
    ...
    // разгънат списък → всичко на цял ред (css: .pl-rz-body.is-all). Пише се САМО при разлика.
    const цял = всички && данни.length > В_СПИСЪКА;
    if (тяло.classList.contains('is-all') !== цял) тяло.classList.toggle('is-all', цял);
    скрий(гр, празно);
    ...
    if (Ш >= 120) {
      const р = графиката(вГр, к, Ш, с.querySelector('.pl-rz-body').id, избран, с);
      гр.innerHTML = р.svg;
      гр._plPts = р.точки;
    } else { с._plSig = ''; }
    ...
    с.querySelector('.pl-rz-list').innerHTML = видими.map(д =>
      '<li><button type="button" class="pl-rz-row pl-soft" data-d="' + д.d + '" aria-pressed="' + (д.d === избран ? 'true' : 'false') + '" aria-label="' + пълна(д.d) + ' — ' + число(к, д.v) + ' ' + М.ед + '">' +
        // 🪤 редът е ДВЕ части (css: .pl-rz-row > span е flex): датата се свива, числото е flex:none
        //    и не се реже никога. Разделителят е интервалът между тях (gap), не буква — затова
        //    вътре няма изричен интервал; четецът така или иначе чете aria-label отгоре.
        '<i aria-hidden="true"></i><span><span class="pl-rz-rd">' + кратка(д.d) + ' ·</span><b>' + число(к, д.v) + ' ' + М.ед + '</b></span><em aria-hidden="true">›</em></button></li>').join('');
