...
const ASSETS = [
  ...
  'img/art/fei/mila.webp',
  'img/art/fei/mira.webp',
  'img/art/fei/malina.webp',
  'img/art/fei/vita.webp',
  'img/art/fei/luna.webp',
  'img/art/fei/iskra.webp',
  'img/art/fei/dara.webp',
  'img/art/fei/niya.webp',
  ...
  'img/art/stai/zdrave.webp',
  'img/art/stai/dnevnik.webp',
  'img/art/stai/igri.webp',
  'img/art/stai/instrumenti.webp',
  'img/art/stai/zhenata.webp',
  'img/art/stai/laboratoriya.webp',
  'js/pl-poyava.js',
  'js/pl-krai.js',
  'css/pl-shrift.css',
  'css/pl-zamama.css',
  ...
  'css/pl-hranene.css',
  'js/pl-hranene.js',
  'css/pl-instrumenti.css',
  'js/pl-instrumenti.js',
  'css/pl-igri.css',
  'js/pl-igri.js',
  'css/pl-profil.css',
  'js/pl-profil.js',
  ...
  'img/logo.png',
  'img/art/hero-home.webp',
  'img/art/welcome.webp',
  'img/art/rb-preg.webp',
  'img/art/rb-baby.webp',
  'img/art/rb-feed.webp',
  'img/art/rb-health.webp',
  'img/art/rb-diary.webp',
  'img/art/rb-play.webp',
  'img/art/rb-tools.webp',
  'img/art/rb-mom.webp',
  'img/art/rb-lab.webp',
  'img/art/ico-a.webp',
  'img/art/ico-b.webp',
  ...
  'img/art/room-feed.webp',
  'img/art/room-health.webp',
  'img/art/room-diary.webp',
  'img/art/room-play.webp',
  'img/art/room-tools.webp',
  'img/art/room-mom.webp',
...
// ⏱️ 22.09: колко чака навигацията мрежата, преди да даде запазената страница (виж клона за навигация)
const НАВ_ТАЙМАУТ = 3000;
...
self.addEventListener('fetch', (e) => {
  ...
  if (навигация) {
    ...
    // ⏱️ 22.09 (анализ на бавния старт — мерено в headless Chromium с истинския SW): мрежата
    //   нямаше ТАВАН. HTML, забавен 5 с (слаб сигнал), = старт след 5 с — точно „3 през нощта,
    //   слаб сигнал“, за който е писана стратегията. Сега: мрежата се пуска веднага; ако до
    //   НАВ_ТАЙМАУТ не отговори, майката получава запазената страница, а мрежата продължава
    //   и опреснява кеша във фона (следващото отваряне е новото). Без запазена страница
    //   (първо отваряне) — чакаме мрежата, както преди. 503 → кеш и пренасочване — непокътнати.
    //   ПЪТ НАЗАД: git revert на комита, после node dev/vdigni_versii.js --pishi.
    const мрежа = fetch(req);
    // клонирането за кеша става ПРЕДИ отговорът да стигне до страницата (редът на .then)
    e.waitUntil(мрежа.then(res => { if (res.ok) пазиСтраницата(req, res); }).catch(() => {}));
    e.respondWith(new Promise(готово => {
      let решено = false;
      const дай = r => { if (!решено && r) { решено = true; готово(r); } };
      const таймер = setTimeout(() => { страницаОтКеша().then(r => { if (r) дай(r); }); }, НАВ_ТАЙМАУТ);
      мрежа.then(res => {
        clearTimeout(таймер);
        if (res.ok || res.type === 'opaqueredirect') return дай(res);
        return страницаОтКеша().then(r => дай(r || res));
      }).catch(() => {
        clearTimeout(таймер);
        страницаОтКеша().then(r => дай(r || Response.error()));
      });
    }));
    return;
  }
