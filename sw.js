...
async function пренеси(нов) {
  ...
  try {
    ...
    for (const име of имена) {
      ...
      const ключове = await стар.keys();
      for (const req of ключове) {
        if (req.url.indexOf(ДОКЛАД) > -1) continue;      // докладът се пише наново
        try {
          if (await нов.match(req)) continue;             // вече е тук
          const res = await стар.match(req);
          if (!res) continue;
          await нов.put(req, res.clone());
          пренесени++;
        } catch (e) { пропуснати++; }
      }
    }
  } catch (e) { /* няма стар кеш или няма достъп — install просто тегли всичко */ }
  return { пренесени: пренесени, пропуснати: пропуснати };
}
  ...
  e.waitUntil((async () => {
    ...
    if (паднали.length) {
      ...
      console.log('[Baby Land SW] офлайн готов: ' + ASSETS.length + ' адреса в „' + CACHE +
        '" · пренесени ' + пренос.пренесени + ' · изтеглени ' + липсващи.length);
    }
  })());
  self.skipWaiting();
});
