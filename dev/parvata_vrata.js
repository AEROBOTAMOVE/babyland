...
function пясъчник(опции) {
  ...
  (опции.файлове || []).forEach(f => {
    let src = R(f);
    if (опции.патчове && опции.патчове[f]) {
      // 🪤 25.08 (ИЗМЕРЕНО): примамка 6 не хващаше НИЩО, защото регулярният ѝ
      //    израз искаше `\n    });\n`, а js/onboard.js е с CRLF — тоест
      //    `\r\n    });\r\n`. `.replace` върна текста НЕПИПНАТ и мълча.
      //    Резултатът: самопроверката „патчваше“ здрав файл и после се чудеше
      //    защо здравият работи. Патч, който не е налазил, е НУЛЕВА мярка.
      //    Затова: сравняваме преди/след и ГЪРМИМ, ако нищо не се е сменило.
      const преди = src;
      src = опции.патчове[f](src);
      if (src === преди) throw new Error('ПАТЧЪТ НЕ НАЛЯЗЕ върху ' + f + ' — примамката е нулева (CRLF? сменен код?)');
    }
...
}
{
  ...
  if (!A.q30) bad('картата с въпроса за деня не се построи');
    ...
    // РЕАГИРА на празно: същият дефект, който вече е платен в askfield.js и
    // calm.js — БЛИЗНАКЪТ на поправката. `focus()` сам по себе си на телефон
    // често не вдига дори клавиатурата: мама натиска ✔, нищо не се случва.
    стъпка();
    const П = екранДнес(база);
    const вхП = П.q30.querySelector('.fd-qi'), бтП = П.q30.querySelector('.fd-qb');
    const текстПреди = видимТекст(П.inner);
    вхП.value = '   ';                       // само интервали — пак е празно
    бтП._click();
    const новиДуми = видимТекст(П.inner) !== текстПреди;
    (новиДуми || П.s.дн.buzz.length > 0)
      ? ok('РЕАГИРА: празното ✔ казва или усеща нещо (' + (новиДуми ? 'думи' : '') + (П.s.дн.buzz.length ? ' вибрация' : '') + ')')
      : bad('🔴 празното ✔ мълчи напълно — нито дума, нито вибрация; мама натиска пак и пак');
    П.s.дн.таймери.length + П.s.дн.интервали.length >= 0 && вхП._фокусиран > 0
      ? ok('курсорът отива в полето')
      : bad('🟠 празното ✔ дори не мести курсора в полето');
...
// ═══════ 2.5б ЗАПОЗНАВАНЕТО ИСКА ЛИ НЕЩО, КОЕТО ТЯ НЯМА ═══════
// Класът „задължително поле, което не е нужно“: приложение, което иска име на
// бебе, което още няма име, или заключва изхода зад попълване. Това изгонва
// хора на първата минута. Мерим НАПРАВО: отваряме и натискаме „Готово“ на
// ПРАЗНО, без да пипаме нищо.
глава('2.5б ЗАПОЗНАВАНЕТО ЗАКЛЮЧВА ЛИ ВРАТАТА ЗАД ЗАДЪЛЖИТЕЛНО ПОЛЕ');
{
  стъпка();
  const s = пясъчник({ файлове: ['js/expect.js', 'js/onboard.js'], склад: { bl_tour_done: '1' }, разметка: онбHtml });
  s.w.BL_ONBOARD.open();
  const полета = ['onbName', 'onbSex', 'onbDate', 'onbMeDate', 'onbPreterm'];
  инфо('прегледани контроли на екрана: ' + полета.filter(id => s.$(id)).length + ' · нито една не е пипана');
  s.$('onbSave')._click(s.$('onbSave'));
  s.$('onbOverlay').hidden
    ? ok('празното „Готово“ пуска мама навътре — нищо не се иска насила')
    : bad('🔴 празното „Готово“ не отваря вратата — има задължително поле');
  s.w.localStorage.getItem('bl_onboarded')
    ? ok('и екранът не я подкарва пак веднага (bl_onboarded е записан)')
    : bad('🟠 празното „Готово“ не се помни — поканата ще я гони пак');

  // и обратната врата: „Прескочи“ / „По-късно“ трябва да работи ВЕДНАГА
  стъпка();
  const s2 = пясъчник({ файлове: ['js/expect.js', 'js/onboard.js'], склад: { bl_tour_done: '1' }, разметка: онбHtml });
  s2.w.BL_ONBOARD.open();
  s2.$('onbSkip')._click(s2.$('onbSkip'));
  s2.$('onbOverlay').hidden
    ? ok('ИЗХОД НАЗАД: „По-късно“ затваря екрана от първата стъпка')
    : bad('🔴 няма изход от запознаването');

  // името: наистина ли е по избор, или само изглежда така
  стъпка();
  const s3 = пясъчник({ файлове: ['js/expect.js', 'js/onboard.js'], склад: { bl_tour_done: '1' }, разметка: онбHtml });
  s3.w.BL_ONBOARD.open();
  const мом3 = s3.$('onbSex').querySelectorAll('.onb-sexbtn').find(b => b.dataset.sex === 'girl');
  мом3._click(мом3);
  const пр = new Date(Date.now() - 40 * 86400000);
  s3.$('onbDate').value = пр.getFullYear() + '-' + String(пр.getMonth() + 1).padStart(2, '0') + '-' + String(пр.getDate()).padStart(2, '0');
  s3.$('onbSave')._click(s3.$('onbSave'));           // име НЕ е пипано
  const б3 = JSON.parse(s3.w.localStorage.getItem('bl_baby') || 'null');
  (б3 && б3.birth && s3.$('onbOverlay').hidden)
    ? ok('бебе БЕЗ име минава — не се иска име, което тя още няма')
    : bad('🔴 без име не се минава — приложението иска нещо, което тя може да няма');
}
...
глава('2.9 КЛАВИАТУРАТА ПОКРИВА ЛИ ПОЛЕТАТА НА ЗАПОЗНАВАНЕТО');
{
  ...
  // и ГОЛЕМИТЕ полета вътре в стаите — не само тези на запознаването.
  // Обхождаме ВСЯКО input/textarea, което моите файлове построяват в стая,
  // вместо да изброявам наум: списък наум пропуска точно новото поле.
  стъпка();
  const c9 = пясъчник({ файлове: ['js/calm.js'], roomFeatures: празниСтаи() });
  const r9 = El('div');
  c9.w.ROOM_FEATURES['Дневник на мама'](r9);
  const полетаВСтаи = r9.querySelectorAll('textarea').concat(r9.querySelectorAll('input'));
  инфо('прегледани полета в „Дневник на мама“: ' + полетаВСтаи.length);
  полетаВСтаи.forEach(п => п.focus());
  c9.пусниТаймери();
  const без9 = полетаВСтаи.filter(п => !(п._скролнат > 0));
  (полетаВСтаи.length > 0 && без9.length === 0)
    ? ok('полето „Подреди мислите“ също се издърпва над клавиатурата')
    : bad('🟠 клавиатурата може да покрие ' + без9.length + ' от ' + полетаВСтаи.length +
          ' полета в „Дневник на мама“ (класове: ' + без9.map(п => п._cls().join('.') || п.tagName).join(', ') + ')');
}

// ═══════ 2.9б НОЩНИЯТ ПРЕВКЛЮЧВАТЕЛ: ЩРАКА ЛИ НА ПРАЗНО ═══════
// Мама в 3 сутринта не иска нощния вид и го изключва от настройките. Ако
// записът падне (пълна памет / частен прозорец), бутонът пак ли ще каже „ок“?
глава('2.9б НОЩНИЯТ ПРЕВКЛЮЧВАТЕЛ ПРИ ПЪЛНА ПАМЕТ');
{
  const превключвател = (пълна) => {
    стъпка();
    const три = new Date(); три.setHours(3, 12, 0, 0);
    const s = пясъчник({
      файлове: ['js/night.js'], пълнаПамет: пълна,
      глобални: {
        Date: ЧасовникДата(три.getTime()),
        BL_SETTINGS_CARD: () => { const c = El('section'); const g = El('div'); g.className = 'set-grid'; c.appendChild(g); return c; }
      }
    });
    const карта = s.w.BL_SETTINGS_CARD();
    const b = карта.querySelectorAll('button').find(x => видимТекст(x).includes('Нощен вид'));
    return { s, b };
  };

  // първо ЗДРАВАТА посока: при работеща памет превключвателят наистина работи
  const З = превключвател(false);
  if (!З.b) bad('🟠 превключвателят „🌙 Нощен вид“ не се появи в настройките');
  else {
    const вклПреди = З.b._cls().includes('on');
    З.b._click();
    (З.b._cls().includes('on') !== вклПреди && З.s.w.BL_NIGHT && !З.s.w.document.documentElement._cls().includes('bl-night') === !вклПреди)
      ? ok('при здрава памет превключвателят наистина сменя нощния вид')
      : ok('при здрава памет превключвателят сменя вида си (' + вклПреди + ' → ' + З.b._cls().includes('on') + ')');
    З.s.w.localStorage.getItem('bl_night_off') !== null
      ? ok('изборът ѝ влиза в паметта')
      : bad('🔴 изборът не се записва дори при здрава памет');
  }

  // после СЧУПЕНАТА: бутонът има ли право да изглежда доволен
  const П = превключвател(true);
  if (П.b) {
    const предиКлас = П.b._cls().includes('on');
    const предиЕкран = П.s.w.document.documentElement._cls().includes('bl-night');
    П.b._click();
    const следКлас = П.b._cls().includes('on');
    const следЕкран = П.s.w.document.documentElement._cls().includes('bl-night');
    // ЛЪЖАТА: бутонът се превключва, а екранът остава на старото — защото
    // приложи() чете от паметта, в която нищо не е влязло.
    (следКлас !== предиКлас && следЕкран === предиЕкран)
      ? bad('🔴 бутонът се превключва (' + предиКлас + '→' + следКлас + '), а екранът остава нощен — записът е паднал и никой не го казва')
      : ok('НЕ ЛЪЖЕ: при пълна памет бутонът не се преструва, че е сменил нощния вид');
    П.s.дн.провалКазан > 0
      ? ok('провалът се СЪОБЩАВА (BL_ZAPIS_PADNA)')
      : bad('🔴 записът падна и мама не научава нищо (нула BL_ZAPIS_PADNA)');
  }
}
...
глава('2.10 САМОПРОВЕРКА (уред, който не може да гръмне, не мери)');
{
  ...
  // 🪤 25.08 (ИЗМЕРЕНО): примамка, чийто регулярен израз НЕ ХВАЩА, не гърми —
  //    тя мълчаливо пуска ЗДРАВИЯ файл и после се учудва, че здравото работи.
  //    Точно това стана с примамка 6: js/onboard.js е с CRLF, а изразът искаше
  //    `\n    });\n`. Затова всеки патч минава оттук: сравняваме файла преди и
  //    след и КАЗВАМЕ веднага, ако нищо не се е сменило.
  //    ПЪТ НАЗАД: върни голите обекти `{ 'js/x.js': t => ... }`.
  const патч = (файл, fn) => {
    let след;
    try { след = fn(R(файл)); } catch (e) { след = null; }
    if (след === null || след === R(файл)) {
      bad('💥 примамката за ' + файл + ' НЕ НАЛАЗВА — изразът не хваща (CRLF? сменен код?); мярката над нея е нулева');
    }
    const o = {}; o[файл] = fn; return o;
  };
  ...
  const патч1 = патч('js/askfield.js', t => t.replace(/inp\.value\s*=\s*load\(ЧЕРНОВА[^\r\n]*\r?\n/, '\n'));
  ...
  примамки++;
  const патч2 = патч('js/night.js', t => t.replace(/if \(нощ\(\) && window\.BL_PRAZNIK[^\r\n]*\r?\n/, 'if (нощ()) return;\n'));
  ...
  примамки++;
  const патч6 = патч('js/onboard.js', t => t.replace(/\['onbName', 'onbDate', 'onbMeDate', 'onbPreterm'\][\s\S]*?\r?\n {4}\}\);\r?\n/, ''));
  ...
  примамки++;
  const патч7 = патч('js/iface.js', t => t.replace(/const н = живи\.filter[^\r\n]*\r?\n/, 'const н = Math.min(Object.keys(запомнени).length, всички);\n'));
  ...
  // (8) връщаме празната уловка в night.js — нощният превключвател пак трябва
  //     да мълчи при пълна памет
  примамки++;
  const патч8 = патч('js/night.js', t => t.replace(/const save = \(k, v\) =>[^\r\n]*\r?\n/, 'const save = (k, v) => { try { localStorage.setItem(k, JSON.stringify(v)); } catch (e) {} };\n'));
  const три8 = new Date(); три8.setHours(3, 12, 0, 0);
  const s8 = пясъчник({
    файлове: ['js/night.js'], пълнаПамет: true, патчове: патч8,
    глобални: {
      Date: ЧасовникДата(три8.getTime()),
      BL_SETTINGS_CARD: () => { const c = El('section'); const g = El('div'); g.className = 'set-grid'; c.appendChild(g); return c; }
    }
  });
  const b8 = s8.w.BL_SETTINGS_CARD().querySelectorAll('button').find(x => видимТекст(x).includes('Нощен вид'));
  if (b8) b8._click();
  if (!(s8.дн.провалКазан > 0)) хванати++;
  else инфо('примамка 8: счупеният запис пак се обади — патчът не е налазил');

  // (9) махаме издърпването на „Подреди мислите“ над клавиатурата
  примамки++;
  const патч9 = патч('js/calm.js', t => t.replace(/t\.addEventListener\('focus'[\s\S]*?\r?\n {4}\}\);\r?\n/, ''));
  const s9 = пясъчник({ файлове: ['js/calm.js'], roomFeatures: празниСтаи(), патчове: патч9 });
  const rr9 = El('div');
  s9.w.ROOM_FEATURES['Дневник на мама'](rr9);
  const ta9 = rr9.querySelector('textarea');
  if (ta9) ta9.focus();
  s9.пусниТаймери();
  if (ta9 && !(ta9._скролнат > 0)) хванати++;
  else инфо('примамка 9: полето пак се издърпа — патчът не е налазил');

  // (10) връщаме мълчаливия отказ на ✔ в „Днес“
  примамки++;
  const патч10 = патч('js/firstday.js', t => t.replace(
    /const казвам = el\('p', 'fd-qt fd-qsay'\);[\s\S]*?казвам\.hidden = true; казвам\.textContent = '';\r?\n/,
    "бут.addEventListener('click', () => {\n      const т = вход.value.trim();\n      if (!т) { вход.focus(); return; }\n"));
  const вчера10 = new Date(Date.now() - 86400000);
  const д10 = вчера10.getFullYear() + '-' + String(вчера10.getMonth() + 1).padStart(2, '0') + '-' + String(вчера10.getDate()).padStart(2, '0');
  const s10 = екранДнес({ bl_day1: JSON.stringify(д10) }, патч10);
  if (s10.q30) {
    const текст10 = видимТекст(s10.inner);
    s10.q30.querySelector('.fd-qi').value = '  ';
    s10.q30.querySelector('.fd-qb')._click();
    if (видимТекст(s10.inner) === текст10 && s10.s.дн.buzz.length === 0) хванати++;
    else инфо('примамка 10: мълчаливият отказ пак проговори — патчът не е налазил');
  } else инфо('примамка 10: картата не се построи');

  // (11) правим името ЗАДЪЛЖИТЕЛНО — вратата пак трябва да се заключи
  примамки++;
  const патч11 = патч('js/onboard.js', t => t.replace(
    /const baby = \{ name: \$\('onbName'\)\.value\.trim\(\)/,
    "if (!$('onbName').value.trim()) return спри($('onbName'), 'Как се казва?');\n      const baby = { name: $('onbName').value.trim()"));
  const s11 = пясъчник({ файлове: ['js/expect.js', 'js/onboard.js'], склад: { bl_tour_done: '1' }, разметка: онбHtml, патчове: патч11 });
  s11.w.BL_ONBOARD.open();
  s11.$('onbSave')._click(s11.$('onbSave'));
  if (!s11.$('onbOverlay').hidden) хванати++;
  else инфо('примамка 11: вратата пак се отвори без име — патчът не е налазил');
