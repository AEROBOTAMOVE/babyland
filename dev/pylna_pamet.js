...
// 🪤 БЕЗ флага `i` — НАРОЧНО. С него „готово:" от заглавието „Преди датата искам
//    да е готово:" се броеше за потвърждение и раждаше находки от въздуха
//    (измерено: 4 фалшиви в js/women4.js). Думите, с които приложението
//    потвърждава на мама, са с ГЛАВНА буква; малката е част от изречение.
const УСПЕХ_ТЕКСТ = /✔|✅|Запечат|Заключен|Записано|Записах|Запазено|Прибрано|Прибрах|Готово|Добавено|Добавих|Махнато|Махнах|Промених|В календара|Добре дошл/;
...
// 🪤 НЕ голото `textContent`: то лепи съседните възли без интервал и ражда
//    думи-кентаври („готово:+Прието."), които ги няма в текста ОТПРЕДИ — тоест
//    всяко натискане изглеждаше, че е добавило нещо ново. Разделяме по възел.
const текстНа = к => {
  const парчета = [];
  (function обходи(в) {
    if (!в) return;
...
async function натисни(W, стая, вид, n) {
  ...
  const предиСклад = JSON.stringify(W.__склад);
  ГРЕШКИ.length = 0;
  ...
  return {
    надпис, файлове,
    писа: предиСклад !== JSON.stringify(W.__склад),
    успех: УСПЕХ_ТЕКСТ.test(ново),
  ...
  for (const стая of СТАИ) {
    ...
    for (const вид of ['бутон', 'поле']) {
      for (let i = 0; i < броеве[вид]; i++) {
        ...
        // 🪤 БЕЗ ЗАПИС НЯМА ЛЪЖА ЗА ЗАПИС. „🎡 Дай ни идея!" не пише нищо — само
        //    рисува карта, чийто БУТОН се казва „✔ Направихме я". Уредът виждаше
        //    новото „✔" и го обявяваше за потвърждение. Затова се иска и склада
        //    наистина да е мръднал в НОРМАЛНИЯ свят (там памет има колкото искаш).
        //    Измерено: това махна 1 измислена находка в js/dev.js и 1 в js/rooms7.js.
        const обещава = (н.успех || н.изчистено) && н.писа;
        if (!обещава) continue;
        обещаващи++;
...
// ═══════════════════════════════════════════════════════════
const ТЕЛА = {
  счупена: "save('bl_proba', поле.value); поле.value = ''; вест.textContent = 'Записано';",
  здрава: "if (!save('bl_proba', поле.value)) { вест.textContent = 'Не можах да го запазя.'; return; } поле.value = ''; вест.textContent = 'Записано';",
  // не пише НИЩО, но показва „✔" — както „🎡 Дай ни идея!" в js/dev.js рисува
  // карта с бутон „✔ Направихме я". Това НЕ е лъжа за запис: запис няма.
  безЗапис: "вест.textContent = '✔ Направихме я';"
};
const ПРОБНА_КАРТА = вид => `
  window.__ПРОБА = function (root, document) {
    ...
    б.addEventListener('click', function () { ${ТЕЛА[вид]} });
...
async function самопроверка() {
  ...
  for (const [счупена, очаква, име] of [
    ['счупена', true, 'ХВАЩА счупеното (голо save + „Записано" + изчистено поле)'],
    ['здрава', false, 'ПРОПУСКА здравото (проверен отговор → мълчи и пази полето)'],
    ['безЗапис', false, 'ПРОПУСКА „✔" БЕЗ запис (няма запис → няма лъжа за запис)']
  ]) {
    const мери = async бездънно => {
      ...
      const предиТ = текстНа(к), предиП = снимкаПолета(к), предиС = JSON.stringify(W.__склад);
      ГРЕШКИ.length = 0;
      бутони(к)[0].click();
      await W.__тик(100);
      ...
      return {
        писа: предиС !== JSON.stringify(W.__склад),
        успех: УСПЕХ_ТЕКСТ.test(ново),
