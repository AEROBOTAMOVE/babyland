...
// 🪤 БРОЯЧЪТ НАДУВАШЕ. Правата кавичка в КОМЕНТАР отваряше низ и текстът на
// коментара се броеше за ключ: 14 фантома (напр. „ отиваше при lb-noi ...").
// Измерено 12.08: 4659 „ключа" срещу 4647 истински. Затова коментарите падат
// ПРЕДИ броенето. Ходи се знак по знак, за да не сгреши „https://" за коментар.
function безКоментари(s) {
  let из = '', i = 0, n = s.length;
  while (i < n) {
    const c = s[i], d = s[i + 1];
    ...
    if (c === "'" || c === '"' || c === '`') {
      из += c; i++;
      while (i < n && s[i] !== c) { if (s[i] === '\\') { из += s[i]; i++; } из += s[i]; i++; }
      из += c; i++; continue;
    }
    из += c; i++;
  }
...
{
  const s = безКоментари(fs.readFileSync('js/kb.js', 'utf8'));
  const ids = new Set();
