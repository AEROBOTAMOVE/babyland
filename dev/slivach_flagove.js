...
// 🔴 16.09 · УРЕДЪТ БЕШЕ СЛЯП ЗА ПРОФИЛА. Помощникът питаше BL_PREGFLAG БЕЗУСЛОВНО, тоест мереше
//   „щеше ли да вика, ако жената беше БРЕМЕННА". Затова фраза, която вика само при бременна, се
//   отказваше с „вече вдига preg през друг път" — а за РОДИЛА жена оставаше ТИШИНА.
//   Хванато на „напоих подложката за час" (следродилен кръвоизлив): pregFlags я улучва,
//   mamaBodyFlags не, и probi_h10 показваше спокойна карта вместо тревога.
//   СЕГА вратата за бременност се брои САМО когато фразата отива в pregFlags.
//   ПЪТ НАЗАД: върни подписа на (W, т) и махни условието на реда с BL_PREGFLAG.
const флаг = (W, т, списък) => {
  try {
    if (W.BL_REDFLAG(т)) return 'red';
    const m = W.BL_MOTHERFLAG(т); if (m) return m;
    if ((!списък || списък === 'pregFlags') && W.BL_PREGFLAG(т, 'Бременност')) return 'preg';
  } catch (e) { return 'err'; }
  return '';
};
...
// 🔎 ПРЕГЛЕДАНИ С ЧЕТЕНЕ И ПРИЕТИ (05.09.2026)
//   Поясът приема всяко заглавие и всеки ключ за НЕВИННО изречение. За
//   картите за СПЕШНОСТИ това е грешно: заглавието им Е спешността.
//   „насини се зад ухото" е знак за счупена черепна основа, а не невинна
//   синина. Тези осем ги прочетох едно по едно; всяко наистина трябва да
//   вдига тревога и досега НЕ я е вдигало — тоест новите флагове ПОПРАВЯТ
//   пропуск, а поясът ги брои за щета само защото са в речника на базата.
//   ⚠️ Списъкът е ИМЕНЕН и кратък нарочно: всяко ново изречение извън него
//   пак спира сливането. Летвата не пада, вдига се именувано изключение.
const ПРИЕТИ_НОВИ_ТРЕВОГИ = new Set([
  "Зениците не са еднакви",          // анизокория
  "устата му се изкриви",            // лицева асиметрия
  "усмивката му е накриво",          // същото
  "не мога да го свестя",            // липса на съзнание
  "изгуби се за няколко секунди",    // абсанс/загуба на съзнание
  "насини се зад ухото",             // знак за счупена черепна основа
  "захарта не пада",                 // неовладян гестационен диабет
  "захарта на гладно е висока"       // същото
]);

const нови = [];
for (const [т, п] of ПРЕДИ) { const сл = СЛЕД.get(т); if (!п && сл && !ПРИЕТИ_НОВИ_ТРЕВОГИ.has(т)) нови.push([т, сл]); }
console.log('');
