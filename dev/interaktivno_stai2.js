...
function опис() {
  ...
  for (const f of МОИ) {
    ...
    while ((m = РЕ.exec(код))) {
      ...
      const пазиУспех = /if\s*\(\s*!?\s*save\s*\(/.test(тяло) || /save\s*\([^;]*\)\s*(?:\?|===|!==)/.test(тяло);
...
async function самопроверка() {
  ...
  // B. описът брои ли това, което наистина е там
  {
    ...
    пробвай('Опис: js/rooms2.js дава 62 слушателя (grep -o, не grep -c)', r2 && r2.брой === 62, r2 ? 'даде ' + r2.брой : 'липсва');
    пробвай('Опис: общо 95 слушателя в 5-те файла (62+7+12+6+8)', о.общо === 95, 'даде ' + о.общо);
