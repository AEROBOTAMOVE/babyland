...
(function () {
  ...
  function фонПод(el) {
    ...
    return { кандидати: канд, картинка: картинка, неясен: неясен, срязано: срязано,
             полупрозрачно: полупрозрачно, opacity: opacityНаЕл };
  }
  ...
  function провериКонтраст(корени) {
    var р = {
      прегледани: 0, ЧЕРВЕНИ: [], ЖЪЛТИ: [], през_полупрозрачно: 0,
    ...
    възлиОт(корени, '*').forEach(function (el) {
      ...
      var запис = {
        ...
        цвят: вЗапис(текстовЦвят), фон: вЗапис(найЛошФон)
      };
  ...
  async function провери(опции) {
    опции = Object.assign({ пауза: 900, паузаТема: 420, теми: ['light', 'dark'],
                            разкрий: true, замразявай: true }, опции || {});
    ...
    for (var т = 0; т < опции.теми.length; т++) {
      ...
      for (var i = 0; i < екрани.length; i++) {
        ...
        var грешка = null;
        try { грешка = await е.отвори(опции); } catch (ex) { грешка = 'отварянето гръмна: ' + ex.message; }
