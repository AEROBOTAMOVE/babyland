...
// 🔴 09.09 — ТОЗИ СПИСЪК БЕШЕ ПРЕПИСАН ТУК И МЯРКАТА ОСТАРЯВАШЕ МЪЛЧАЛИВО.
//   Добавих 46 id в helper.js и числото НЕ МРЪДНА — защото сондата четеше
//   своето копие. Мярка, която носи собствено копие на правилото, мери
//   вчерашния ден. Сега се чете ОТ ИЗТОЧНИКА.
const _изв = require('fs').readFileSync('js/helper.js', 'utf8');
const _бл = /const БЕЗ_ПОДАВАНЕ = \[([\s\S]*?)\];/.exec(_изв);
if (!_бл) { console.log('🔴 не намирам БЕЗ_ПОДАВАНЕ в helper.js — СЛЯПА МЯРКА'); process.exit(2); }
const БЕЗ_ПОДАВАНЕ = (_бл[1].match(/'[a-z0-9-]+'/g) || []).map(x => x.slice(1, -1));
if (БЕЗ_ПОДАВАНЕ.length < 13) { console.log('🔴 прочетени само ' + БЕЗ_ПОДАВАНЕ.length + ' id'); process.exit(2); }
