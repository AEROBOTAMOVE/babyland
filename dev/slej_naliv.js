...
for (const f of файлове) {
  ...
  for (const x of данни) {
    ...
    if (заглавияСега.has(норм(заг))) { отказ.push('🔶 ' + f + ' „' + заг.slice(0, 40) + '" — ВЕЧЕ СЪЩЕСТВУВА в библиотеката'); continue; }
    if (!домНаСтая[стая]) { отказ.push('🔴 ' + f + ' — не знам в кой файл живее стая „' + стая + '"'); continue; }
    заглавияСега.add(норм(заг));
    приети.push({ f, стая, заг, тяло: кав(тяло), x });
  }
}
